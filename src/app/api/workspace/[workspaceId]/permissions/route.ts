...
import { getPermissionQueueCollection } from "@/lib/mongodb";
...
export async function GET(
  ...
  try {
    ...
    const collection = await getPermissionQueueCollection(dbName);
    ...
    return NextResponse.json(
      { error: "Failed to fetch permissions status breakdown" },
      { status: 500 }
    );
  }
}
